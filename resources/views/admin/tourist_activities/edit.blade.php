...
@section('header')
    <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
        {{ __('Edit Activity') }}: {{ $touristActivity->name }}
    </h2>
@endsection
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg-px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('admin.tourist-activities.update', $touristActivity) }}" method="POST">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                             ...
                             <div>
                                ...
                                <select id="site_id" name="site_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    ...
                                    @foreach ($sites as $site)
                                         <option value="{{ $site->id }}" {{ old('site_id', $touristActivity->site_id) == $site->id ? 'selected' : '' }}>{{ $site->name }}</option>
                                    @endforeach
                                </select>
                             ...
                             <div>
                                ...
                                <x-text-input id="location_text" name="location_text" type="text" class="mt-1 block w-full" :value="old('location_text', $touristActivity->location_text)" />
                                <x-input-error class="mt-2" :messages="$errors->get('location_text')" />
                            </div>

                             <div>
                                <x-input-label for="start_datetime" :value="__('Start Date and Time')" />
                                {{-- Format existing datetime for datetime-local input (YYYY-MM-DDTHH:mm) --}}
                                @php
                                    $startDatetimeValue = old('start_datetime', $touristActivity->start_datetime ? $touristActivity->start_datetime->format('Y-m-d\TH:i') : '');
                                @endphp
                                <x-text-input id="start_datetime" name="start_datetime" type="datetime-local" class="mt-1 block w-full" :value="$startDatetimeValue" required />
                             ...
                             <div>
                                ...
                                <x-text-input id="duration_minutes" name="duration_minutes" type="number" min="1" class="mt-1 block w-full" :value="old('duration_minutes', $touristActivity->duration_minutes)" />
                             ...
                             <div>
                                ...
                                <select id="organizer_user_id" name="organizer_user_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    ...
                                    @foreach ($organizers as $organizer)
                                         <option value="{{ $organizer->id }}" {{ old('organizer_user_id', $touristActivity->organizer_user_id) == $organizer->id ? 'selected' : '' }}>{{ $organizer->username }} ({{ $organizer->user_type }})</option>
                                    @endforeach
                                </select>
                             ...
                             <div>
                                ...
                                <x-text-input id="price" name="price" type="number" step="0.01" class="mt-1 block w-full" :value="old('price', $touristActivity->price)" />
                                <x-input-error class="mt-2" :messages="$errors->get('price')" />
                            </div>

                             <div>
                                <x-input-label for="max_participants" :value="__('Max Participants (Optional)')" />
                                <x-text-input id="max_participants" name="max_participants" type="number" min="1" class="mt-1 block w-full" :value="old('max_participants', $touristActivity->max_participants)" />
                                <x-input-error class="mt-2" :messages="$errors->get('max_participants')" />
                            </div>

                            {{-- Add image/video file inputs and display if applicable --}}
